...
| `ACCEPTANCE_ENABLE_VERSIONING`                  | Enables object versioning tests. Requires the target server to have `STORAGE_VERSIONING_ENABLED=true` (single-tenant) or the tenant's `objectVersioning` feature flag (multitenant). |
