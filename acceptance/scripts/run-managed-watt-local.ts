...
serverEnv.DATABASE_WATT_TEST_SERVER = 'true'
serverEnv.DATABASE_WATT_TEST_PORT = serverEnv.DATABASE_WATT_TEST_PORT || '5001'
serverEnv.WORKERS_NUM = serverEnv.WORKERS_NUM || '2'
serverEnv.PLT_MANAGEMENT_API = serverEnv.PLT_MANAGEMENT_API || 'false'
...
const databaseWattBaseUrl =
  acceptanceEnv('ACCEPTANCE_DATABASE_WATT_BASE_URL') ??
  `http://127.0.0.1:${serverEnv.DATABASE_WATT_TEST_PORT}`
const acceptanceRunEnv: NodeJS.ProcessEnv = {
  ...process.env,
  ACCEPTANCE_BASE_URL: baseUrl,
  ACCEPTANCE_DATABASE_WATT_BASE_URL: databaseWattBaseUrl,
...
async function main() {
  try {
    ...
    await waitForStatus(`${databaseWattBaseUrl}/stats`, 60_000)
