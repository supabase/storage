...
const upsertObjectParamsSchema = {
  type: 'object',
  properties: {
...
interface upsertObjectRequestInterface extends AuthenticatedRequest {
  Params: FromSchema<typeof upsertObjectParamsSchema>
}

// eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
export default async function routes(fastify: FastifyInstance) {
  const summary = "Upsert the object, creates if it doesn't exist, otherwise updates"

  const schema = createDefaultSchema(successResponseSchema, {
    params: upsertObjectParamsSchema,
    summary,
    tags: ['object'],
  })
  fastify.post<upsertObjectRequestInterface>(
    '/upsert/:bucketName/*',
    {
      schema,
    },
    async (request, response) => {
      ...
      if (objectResponse.error) {
        // Insert operation if the object is not found
        // meaning the object doesn't exist in storage
        if (objectResponse.status === 406) {
          // Status code 406 here means row was no found, hence insert
          ...
          // Throw error for other status code
          const { status, error } = objectResponse
          request.log.error({ error }, 'error object')
          return response.status(400).send(transformPostgrestError(error, status))
        }
      } else {
        // Object exists, Update the last accessed at and owner field
        const updateResponse = await postgrest
          .from<Obj>('objects')
          .update({
        ...
        if (updateResponse.error) {
          const { status, error } = updateResponse
          request.log.error({ error }, 'error object')
          return response.status(400).send(transformPostgrestError(error, status))
        }
      }

      // if successfully inserted or updated, upload the object to s3
