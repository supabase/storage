...
export class ObjectBackup {
  ...
  private async singleCopy(): Promise<void> {
    ...
    const copyParams = {
      ...
      // Optional: Add additional parameters like ACL, Metadata, etc.
    }
  ...
  private async copyPartsConcurrently(
    ...
    const copyPart = async (partNumber: number): Promise<void> => {
      ...
      // console.log(`Copying part ${partNumber} (bytes ${start} to ${end})`)
      ...
      // console.log(
      //   `Part ${partNumber} copied successfully with ETag: ${uploadPartCopyResponse.CopyPartResult.ETag}`
      // )
    }
