...
export class StoragePgDB implements Database {
  ...
  private readonly unavailableSelectColumns: number
  ...
  constructor(
    ...
    const hasUserMetadataColumn =
      !this.latestMigration || DBMigration[this.latestMigration] >= DBMigration['custom-metadata']
    const hasMultipartMetadataColumn =
    ...
    this.unavailableSelectColumns =
      (hasUserMetadataColumn ? 0 : SelectColumnOptions.excludeUserMetadata) |
      (hasMultipartMetadataColumn ? 0 : SelectColumnOptions.excludeMultipartMetadata)
  }
  ...
  async findBucketById(bucketId: string, columns = 'id', filters?: FindBucketFilters) {
    const selectColumnOptions = (await tenantHasMigrations(
      this.tenantId,
      'iceberg-catalog-flag-on-buckets'
    ))
      ? SelectColumnOptions.none
      : SelectColumnOptions.excludeBucketType
    const selectedColumns = selectColumns(columns, selectColumnOptions)
  ...
  async listBuckets(columns = 'id', options?: ListBucketOptions) {
    const selectedColumns = selectColumns(columns, SelectColumnOptions.syntheticBucketType)
  ...
  async findMultipartUpload(uploadId: string, columns = 'id', options?: { forUpdate?: boolean }) {
    const selectedColumns = selectColumns(columns, this.unavailableSelectColumns)
  ...
  protected normalizeRecordColumns<T extends Record<string, unknown>>(columns: T): T {
    if (!(this.unavailableSelectColumns & SelectColumnOptions.excludeUserMetadata)) {
      return columns
    }
  ...
  protected hasMultipartMetadataColumn(): boolean {
    return !(this.unavailableSelectColumns & SelectColumnOptions.excludeMultipartMetadata)
  }
