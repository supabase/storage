...
export class GenerateObjectSignature extends BaseEvent<GenerateObjectSignaturePayload> {
  ...
  static async handle(
    job: Job<GenerateObjectSignaturePayload>,
    ...
    try {
      ...
      if (await objectSignatureJobIsStale(storage, job.data)) {
        logStaleObjectSignatureJob(job, objectPath)
        return
      }

      let response: ObjectResponse
      try {
        response = await storage.backend.getObject(
          storageS3Bucket,
          objectPath,
          job.data.version,
          undefined,
      ...
      const sha256 = await digestObjectBody(response.body, signal)
      throwIfAborted(signal)
      const updated = await storage.db.updateObjectSignature(
        job.data.bucketId,
        job.data.objectName,
        job.data.version,
...
async function objectSignatureJobIsStale(
  storage: Storage,
  payload: GenerateObjectSignaturePayload
) {
  const currentObject = await storage.db.findObject(
    payload.bucketId,
    payload.objectName,
    'version',
    {
  ...
  return !currentObject || !objectVersionMatches(currentObject.version, payload.version)
}

function objectVersionMatches(
  currentVersion: string | null | undefined,
  queuedVersion: string | undefined
) {
  return (currentVersion ?? null) === (queuedVersion ?? null)
}

function logStaleObjectSignatureJob(job: Job<GenerateObjectSignaturePayload>, objectPath: string) {
  logSchema.warning(logger, `[Admin]: GenerateObjectSignature ${objectPath} skipped stale object`, {
    jobId: job.id,
    type: 'event',
    event: 'GenerateObjectSignature',
    payload: JSON.stringify(job.data),
...
async function digestObjectBody(body: ObjectResponse['body'], signal?: AbortSignal) {
  if (!body) {
  ...
  if (body instanceof Blob) {
    await updateHashFromReadableStream(hash, body.stream(), signal)
    return hash.digest('hex')
  }

  if (body instanceof Readable) {
    await updateHashFromIterable(hash, body, signal)
    return hash.digest('hex')
  }

  if (isReadableStream(body)) {
    await updateHashFromReadableStream(hash, body, signal)
    return hash.digest('hex')
  }
...
async function updateHashFromIterable(
  ...
  const abort = () => {
    if (body instanceof Readable) {
      body.destroy(getAbortError(signal))
    }
  }
...
async function updateHashFromReadableStream(
  ...
  const abort = () => {
    reader.cancel(getAbortReason(signal)).catch(() => {
      // no-op: the digest loop observes the same signal and exits with its abort reason
    })
  }
