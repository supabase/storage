...
export class SignatureV4 {
  ...
  static parseAuthorizationHeader(headers: SignatureHeaders) {
    const clientSignature = getSingleValue(headers.authorization, 'authorization')
    ...
    const longDate = getSingleValue(headers['x-amz-date'], 'x-amz-date')
    const contentSha = getSingleValue(headers['x-amz-content-sha256'], 'x-amz-content-sha256')
    const sessionToken = getSingleValue(headers['x-amz-security-token'], 'x-amz-security-token')
...
function getSingleValue(
  ...
  throw ERRORS.InvalidSignature(`Multiple ${headerName} headers are not supported`)
}
