  ...
  switch (op) {
    ...
    case '$lte': {
      if (typeof raw !== 'number') {
        throw ERRORS.InvalidParameter(`${op} requires a number for "${fieldName}"`)
      }
      checkFinite(raw)
    ...
    case '$in': {
      if (!Array.isArray(raw) || raw.length === 0) {
        throw ERRORS.InvalidParameter(`$in requires a non-empty array for "${fieldName}"`)
      }
    ...
    case '$nin': {
      if (!Array.isArray(raw) || raw.length === 0) {
        throw ERRORS.InvalidParameter(`$nin requires a non-empty array for "${fieldName}"`)
      }
      const values = (raw as FilterPrimitive[]).map((v) => primitiveAsText(validatePrimitive(v)))
      const lhs = jsonText(ctx, fieldName)
    ...
    case '$exists': {
      if (typeof raw !== 'boolean') {
        throw ERRORS.InvalidParameter(`$exists requires a boolean for "${fieldName}"`)
      }
...
function translateFieldClause(
  ...
  if (value !== null && typeof value === 'object' && !Array.isArray(value)) {
    ...
    if (ops.length === 0) {
      throw ERRORS.InvalidParameter(`Empty operator object for field "${fieldName}"`)
    }
    for (const [op] of ops) {
...
function translateInternal(ctx: Ctx, filter: S3VectorFilter): string {
  ...
  const clauses = keys.map((name) => {
    if (name.startsWith('$')) {
      throw ERRORS.InvalidParameter(`Unexpected operator "${name}" at field position`)
    }
...
export function translateFilter(filter: S3VectorFilter, column = 'metadata'): TranslatedFilter {
  if (!VALID_COLUMN_REF.test(column)) {
    throw ERRORS.InvalidParameter(`Invalid metadata column reference: ${column}`)
  }
