...
export class VectorStoreManager {
  constructor(
    ...
    protected readonly config: { tenantId: string; vectorBucketName: string }
  ) {}
  ...
  async createBucket(bucketName: string): Promise<void> {
    await this.db.createVectorBucket(bucketName)
  }
  ...
  async deleteIndex(command: DeleteIndexInput): Promise<void> {
    ...
    await this.db.withTransaction(async (tx) => {
      await tx.deleteVectorIndex(command.vectorBucketName!, vectorIndexName)
