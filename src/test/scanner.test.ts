...
describe('ObjectScanner', () => {
  ...
  it('Will delete S3 objects, if no records exists in the database', async () => {
    ...
    const deleted = await storage.scanner.deleteOrphans(bucket.id, options)
    expect(deleted).toHaveProperty('s3OrphanedKeys', objectsToDelete.length)
    expect(deleted).toHaveProperty('dbOrphanedKeys', 0)
